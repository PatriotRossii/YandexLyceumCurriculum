...
def determine_animal(storage, user_id):
    current_state = storage[user_id]["state"]

    if current_state == "elephant":
        return "слон"
    elif current_state == "rabbit":
        return "кролик"
    return None
...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f"Привет! Купи {determine_animal(sessionStorage, user_id) + 'а'}!"
        # Получим подсказки
    ...
    splitted = req['request']['original_utterance'].lower().split()
    for word in [
        'ладно',
        'куплю',
        'покупаю',
        ...
        if word in splitted:
            res['response']['text'] = f"{determine_animal(sessionStorage, user_id).capitalize() + 'а'} можно найти на " \
                                      f"Яндекс.Маркете!"

            next_state = STATE_MACHINE[sessionStorage[user_id]["state"]]
            if next_state is None:
                res['response']['end_session'] = True
            else:
                sessionStorage[user_id]["state"] = next_state
                sessionStorage[user_id]["suggests"] = DEFAULT_SUGGESTS

                res['response']['text'] += f' А теперь купи {determine_animal(sessionStorage, user_id) + "а"}!'
                res['response']['buttons'] = get_suggests(user_id)
            return
    ...
    res['response']['text'] = \
        ...
        f"{determine_animal(sessionStorage, user_id) + 'а'}!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={determine_animal(sessionStorage, user_id)}",
            "hide": True
        })
